...
The `window` function becomes particularly useful when we want to take data from a defined period.  For example, if we decide that data from 1956-1969 is not appropriate due to regulatory changes then we can create a training data set from the 1st qtr of 1970 through the 4th qtr of 1995.
...
What `accuracy` is doing is taking the model output from the training data (`naive_fc` & `mean_fc`) and applying the 108 forecasted values to those values in `gold` that are not included in the model (aka test data). This means we do not need to directly feed it a test data set.
